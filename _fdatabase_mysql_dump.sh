...
function database_mysql_dump()
{
  ...
  _cmd="mysqldump $_mysqldump_options --defaults-file=$_credential_file --databases $_db_name --result-file=$_db_backup_output_file --log-error=$_db_backup_output_file.errors"
  echo "$(date) - [$SCRIPT_NAME ($SCRIPT_PID)] - $_fname - $_task - Start"
